...
def main():
    ...
    if len(sys.argv) == 2:
        gender = sys.argv[1]
    proc = subprocess.Popen(["curl ec2-13-59-238-205.us-east-2.compute.amazonaws.com:80/?gender=" + gender],
                            shell=True, stdout=subprocess.PIPE)
    (out, err) = proc.communicate()
    bearer_token = out.decode("utf-8").strip()

    for activity in os.listdir(boston_activity_dir):
        activity_path = os.path.join(boston_activity_dir, activity)
        ...
        if '"id"' in out:
            ...
            os.rename(os.path.join(boston_activity_dir, activity),
